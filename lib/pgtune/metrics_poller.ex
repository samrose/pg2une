defmodule Pgtune.MetricsPoller do
  @moduledoc """
  ...
  - Pgtune.MetricsStore for persistence
  - Pgtune.WorkloadDetector for classification
  - Pgtune.QueryRegression for per-query analysis
  """
  ...
  def init(opts) do
    ...
    target_url = Keyword.get(opts, :target_url) || Pgtune.Config.target_url()
  ...
  def handle_info(:reconnect, state) do
    target_url = Pgtune.Config.target_url()
  ...
  defp do_poll(state) do
    with {:ok, system_metrics} <- collect_system_metrics(state.conn),
      ...
      Anytune.ingest(:pgtune, system_metrics)

      # Update workload classification
      Pgtune.WorkloadDetector.update(system_metrics)

      # Persist to store
      persist_metrics(system_metrics, query_metrics)

      # Run query regression detection
      Pgtune.QueryRegression.analyze(query_metrics)
  ...
  defp persist_metrics(system_metrics, query_metrics) do
    Pgtune.MetricsStore.record_system_snapshot(system_metrics)

    if query_metrics != [] do
      Pgtune.MetricsStore.record_workload_snapshots(query_metrics)
    end
  end
end
